...
`;
                ...
                const useStrict = `    "${prologue}";`;
                const pos1 = outFile.content.indexOf(useStrict);
                const pos2 = outFile.content.indexOf(useStrict, pos1 + useStrict.length);
                verifyIncrementalWatchEmit({
                    files: [libFile, file1, file2, config],
                    expectedInitialEmit: [
                        outFile,
                        {
                            ...
                            content: getBuildInfoText({
                                bundle: {
                                    ...
                                    js: {
                                        sections: [
                                            { pos: pos1, end: pos1 + useStrict.length, kind: BundleFileSectionKind.Prologue, data: prologue },
                                            { pos: pos2, end: pos2 + useStrict.length, kind: BundleFileSectionKind.Prologue, data: prologue },
                                            { pos: 0, end: outFile.content.length, kind: BundleFileSectionKind.Text }
                                        ]
                                    },
                                }
